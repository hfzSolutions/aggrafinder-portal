...
const CompareTools = () => {
  ...
  const { tools: fetchedTools, loading, error: fetchError } = useSupabaseTools();
  
  useEffect(() => {
    if (!loading && !fetchError) {
      const filteredTools = fetchedTools.filter(tool => toolIds.includes(tool.id));
  ...
  return (
    <>
      <Helmet>
        <title>Compare AI Tools | AI Tools Aggregator</title>
        <meta 
          name="description" 
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-grow pt-20">
          <div className="container px-4 md:px-8 mx-auto py-8">
            ...
            <h1 className="text-3xl md:text-4xl font-semibold mb-2">Compare AI Tools</h1>
            ...
            {isLoading ? (
              <div className="space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {Array(toolIds.length).fill(0).map((_, idx) => (
              ...
              <div className="rounded-lg border border-red-200 p-6 text-center bg-red-50">
                <h2 className="text-xl font-medium mb-2 text-red-600">Error loading tools</h2>
              ...
              <div className="rounded-lg border p-6 text-center">
                <h2 className="text-xl font-medium mb-2">Not enough tools to compare</h2>
                <p className="text-muted-foreground mb-4">Please select at least 2 tools to compare.</p>
                <Button asChild>
              ...
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="hidden md:block"></div> {/* Empty first column on desktop */}
                  
                  {tools.map(tool => (
                    <div key={tool.id} className="bg-card rounded-lg border border-border/60 shadow-sm p-4 flex flex-col items-center">
                      ...
                      <h2 className="text-lg font-medium text-center mb-1">{tool.name}</h2>
                      
                      <div className="mb-3">
                        <span className={cn(
                          "text-xs font-medium px-2 py-1 rounded-full",
                          {
                            "bg-green-100 text-green-800": tool.pricing === "Free",
                            "bg-blue-100 text-blue-800": tool.pricing === "Freemium",
                            "bg-purple-100 text-purple-800": tool.pricing === "Paid",
                            "bg-amber-100 text-amber-800": tool.pricing === "Free Trial",
                            "bg-gray-100 text-gray-800": !["Free", "Freemium", "Paid", "Free Trial"].includes(tool.pricing)
                          }
                        )}>
                          {tool.pricing}
                        </span>
                      ...
                      <p className="text-sm text-muted-foreground text-center mb-4 flex-grow">{tool.description}</p>
