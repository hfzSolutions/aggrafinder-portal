  ...
  const fetchUserTools = async () => {
    try {
      ...
      const tools: AITool[] = data.map((tool) => ({
        ...
        imageUrl: tool.image_url ? `${import.meta.env.VITE_STORAGE_URL}/${tool.image_url}` : '',
    ...
    <div className="space-y-4">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg">
              <p className="mb-4 text-muted-foreground">You haven't submitted any AI tools yet.</p>
                ...
                <div className="flex gap-4">
                  <div className="w-16 h-16 rounded bg-muted flex-shrink-0 overflow-hidden">
                    {tool.imageUrl ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-secondary">
                        <span className="text-xs text-secondary-foreground">No Image</span>
                      </div>
                    )}
                  </div>
                  <div className="flex-grow">
                    <div className="flex justify-between">
                      ...
                      <div>
                        {getStatusBadge(tool.approvalStatus)}
                      </div>
                    </div>
      ...
      <Dialog open={openSubmitDialog} onOpenChange={setOpenSubmitDialog}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingTool ? 'Edit AI Tool' : 'Submit New AI Tool'}</DialogTitle>
            <DialogDescription>
              {editingTool 
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete your tool from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
